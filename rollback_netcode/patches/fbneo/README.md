...
| `0008-ft-end-notice.diff` | `src/burner/win32/run.cpp` | quando a sessão acaba porque a FT combinada chegou ao fim (`FbnHostEndedByLimit()`), mostra `FbnHostShowLimitNotice()` — "FT3 encerrada", placar final, fecha sozinho em 3 s — em vez do aviso de "conexão caiu". Desconexão e falha de sincronia continuam com o aviso antigo. Depende de `0001` |
