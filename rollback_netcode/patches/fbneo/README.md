...
| `0006-dx9-match-bar.diff` | `src/intf/video/win32/vid_directx9.cpp` | desenha a barra da partida **na resolução da janela**, com `ID3DXFont` e `Boxxy` — muito mais nítido que desenhar numa imagem de 384px e deixar o blitter ampliar. Chama `OverlayClaim()` para o desenho na imagem se calar |
| `0005-overlay-names-and-score.diff` | `src/intf/video/vid_interface.cpp` | uma chamada a `OverlayDraw()` no `VidFrameCallback`, logo depois do gancho do Lua — desenha nome e placar **dentro da imagem do jogo**, então vale para todos os blitters e sai também em screenshot |
